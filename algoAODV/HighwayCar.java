...
public class HighwayCar extends Node {
	...
	boolean testCarMobil=false;
	public boolean isTestCarMobil() {
		return testCarMobil;
	}

	public void setTestCarMobil(boolean testCarMobil) {
		this.testCarMobil = testCarMobil;
	}
	 ...
	 * 
	 **********************************/
	public int getNbSautsPourSource(int s)
	{
		for (int i=0; i<vectAodv.size();i++)
		{
			if(vectAodv.get(i).getSource()==s)
				return vectAodv.get(i).getNbSauts();
		}
		return -1;
	}
	...
	public void onStart() {
		...
		if (containstpp()==-1)
		{
			MainAODV.vectPassageTemps.add(tpp);
		}
		
	}
	public int containstpp() { 
		for (int i=0; i<MainAODV.vectPassageTemps.size();i++)
		{
			if(MainAODV.vectPassageTemps.get(i).getId()==this.getID())
			{
				return i;
			}
		}
			...
			for (Node node : getSensedNodes()) {
				...
				if (node instanceof HighwayCar) {
					for (int i=0; i<((HighwayCar) node).getVectAodv().size();i++)
					{
						...
						for (int j=0; j<vectAodv.size();j++)
						{
							if (vectAodv.get(j).getSource()==((HighwayCar) node).getVectAodv().get(i).getSource())
							{
								...
								if(vectAodv.get(j).getNbSauts()>((HighwayCar) node).getVectAodv().get(i).getNbSauts()+1 )
								{
									...
									vectAodv.get(j).setTourElue(((HighwayCar) node).getTourElue());
									if (((HighwayCar) node).getVectAodv().get(i).getDebit()==0)
									{
										vectAodv.get(j).setDebit(this.debitDispo);
									}
									else
									vectAodv.get(j).setDebit(Math.min(this.debitDispo, ((HighwayCar) node).getVectAodv().get(i).getDebit()));
						...
						if (!sourcePresence)
						{
							long debitTour=(long) ((HighwayCar) node).getVectAodv().get(i).getDebit();
							if (debitTour==0)
							{
								debitTour=(long) this.debitDispo;
							}
							vectAodv.add(new Aodv(
									((HighwayCar) node).getVectAodv().get(i).getSource(), 
									((HighwayCar) node).getVectAodv().get(i).getNbSauts()+1,
									(HighwayCar) node,
									this.tourElue,
	...
	public void choixTour() {
		...
		for (int i = 0; i < l_tourDispo.size(); i++) {
				if (tourElue==null)
				{
					...
					MainAODV.vectPassageTemps.get(containstpp()).setNbPassage(this.nbPassage);
					MainAODV.vectPassageTemps.get(containstpp()).setDebit(this.debitDispo);
					MainAODV.vectPassageTemps.get(containstpp()).setOptiPareto(isParetoOptimum());
					MainAODV.vectPassageTemps.get(containstpp()).setTemps(System.currentTimeMillis()-this.tempsDebut);
				}
				if ((tourElue!=null) && 
						(l_tourDispo.get(i).getDebitDisponible()> tourElue.getDebitDisponible())&&
				...
				MainAODV.vectPassageTemps.get(containstpp()).setOptiPareto(isParetoOptimum());
				MainAODV.vectPassageTemps.get(containstpp()).setTemps(System.currentTimeMillis()-this.tempsDebut);
				DebitChgmtTemps vDCTps= new DebitChgmtTemps(MainAODV.debitTotal(), MainAODV.nbPassageTotal(), System.currentTimeMillis()-this.tempsDebut, nbOptiPareto() );
				MainAODV.vectDebitChgmtTemps.add(vDCTps);
			}
		}
		/*
	...
	public void onClock() {
		...
		System.out.println("car num "+this.getID()+ " : "+MainAODV.toStringvpt());
		System.out.println("debit total ="+nbDebitTotal()+" nb sauts"+MainAODV.nbPassageTotal()+ " et nbOptimumPareto = "+nbOptiPareto());
		System.out.println("SORTIE ="+MainAODV.toStringdct());
	...
	public double nbDebitTotal()
	{
		...
		for (int i=0; i<MainAODV.vectPassageTemps.size();i++)
		{
			deb+=MainAODV.vectPassageTemps.get(i).getDebit();
		}
		return deb;
	}
	public int nbOptiPareto()
	{
		...
		for (int i=0; i<MainAODV.vectPassageTemps.size();i++)
		{
			if(MainAODV.vectPassageTemps.get(i).isOptiPareto())
			{
				tot++;
			}
		}
	...
	public String toString() {
		...
		{
			str=str+this.vectAodv.get(i).toString();
		}
